...
class VideoCompressorUnified:
    """
    Node de compression vidéo unifié :
    - Accepte images ou vidéo
    - Trois modes : fast_crf, 2pass, gpu
    """
    ...
    def _get_video_info(self, video_path):
        ...
        if len(parts) < 2:
            raise ValueError("Impossible d'analyser la vidéo.")
    ...
    def _load_video_to_tensor(self, video_path, temp_dir):
        ...
        subprocess.run(
            ["ffmpeg", "-i", video_path, os.path.join(frames_dir, "frame_%05d.png")],
    ...
    def process(self, mode, target_mb, codec, images=None, video_path_in=None, fps=24, crf=23, preset="fast"):
        ...
        try:
            if images is not None:
                ...
                subprocess.run(
                    ["ffmpeg", "-y", "-framerate", str(fps), "-i", os.path.join(temp_dir, "frame_%05d.png"),
                     "-c:v", "libx264", "-pix_fmt", "yuv420p", initial_video_path],
                    check=True, capture_output=True
                )
            else:
                if not os.path.exists(video_path_in):
                    raise FileNotFoundError(f"Fichier vidéo non trouvé: {video_path_in}")
            ...
            if duration_s <= 0:
                raise ValueError("Durée de la vidéo nulle ou invalide.")

            # ----- Choix du mode -----
            if mode == "2pass":
                # Calcule le bitrate cible
                ...
                subprocess.run([
                    "ffmpeg", "-y", "-i", initial_video_path,
                    "-c:v", codec, "-preset", preset, "-crf", str(crf),
                    "-c:a", "aac", "-b:a", "128k", "-threads", "0",
                    final_video_path
                ], check=True, capture_output=True)

            elif mode == "gpu":
                # Ici on force un CRF rapide avec encodeur GPU
                subprocess.run([
                    "ffmpeg", "-y", "-i", initial_video_path,
                    "-c:v", codec, "-preset", preset, "-cq", str(crf),
                    "-c:a", "aac", "-b:a", "128k", "-threads", "0",
                    final_video_path
                ], check=True, capture_output=True)
